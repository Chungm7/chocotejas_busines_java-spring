...
$(document).ready(function() {
    ...
    function initializeDataTable() {
        dataTable = $('#tablaPerfiles').DataTable({
            ...
            columns: [
                ...
                {
                    data: 'estado',
                    render: (data) => data ? '<span class="badge text-bg-success">Activo</span>' : '<span class="badge text-bg-secondary">Inactivo</span>'
                },
                {
                    data: null,
                    orderable: false,
    ...
    function createActionButtons(row) {
        return `
            <div class="d-flex gap-1 justify-content-center">
                <button data-id="${row.id}" class="action-btn action-btn-permissions action-permissions" title="Permisos">
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-shield-lock-fill" viewBox="0 0 16 16">
                        <path d="M5.338 1.59a61.44 61.44 0 0 0-2.837.856.481.481 0 0 0-.328.39c-.554 4.157.726 7.19 2.253 9.188a10.725 10.725 0 0 0 2.287 2.233c.346.244.652.42.893.533.12.056.255.115.385.17.117.05.238.097.36.133.124.037.25.06.377.06s.253-.023.377-.06a2.1 2.1 0 0 0 .745-.265c.16-.085.312-.18.456-.282.14-.1.274-.213.396-.333a10.726 10.726 0 0 0 2.287-2.233c1.527-1.997 2.807-5.031 2.253-9.188a.48.48 0 0 0-.328-.39c-.651-.213-1.75-.56-2.837-.855C9.552 1.29 8.5 1 8 1s-1.552.29-2.662.59zM10 8.5a1.5 1.5 0 0 1-1.5 1.5h-1a1.5 1.5 0 1 1 0-3h1A1.5 1.5 0 0 1 10 8.5z"/>
                    </svg>
                </button>
                ...
                <div class="form-check form-switch">
                    <input class="form-check-input action-status" type="checkbox" role="switch" 
                           data-id="${row.id}" ${row.estado ? 'checked' : ''}
    ...
    function setupEventListeners() {
        ...
        $('#tablaPerfiles tbody').on('change', '.action-status', handleToggleStatus);
